...
function CookieProduct(name, content, price) { 
    ...
    this.price = price;
    // this는 자기자신을 가리킴. 함수밖에서 사용하면 window를 의미
    // 해당 예시의 경우라면 CookieProduct를 가리킴.
}
// => let cookieProduct = new CookieProduct(a, b, c)를 입력하면 
// cookieProduct.name = a, cookieProduct.content = b, cookieProduct.price = c 를 멤버필드로 가지는 cookieProduct가 생성됨.
...
// (1) 주어진 문자열(str)에서 
// (2) 특정 부분 문자열(searchStr)을 찾아서 
// (3) 다른 문자열(replaceStr)로 교체하는 역할을 합니다.
...
window.onload = function() {
    ...
    // cookies 라는 배열에 추가하는 
    // 구문을 작성합니다.
    // (1) 배열 중 cookie_image_files의 배열을 길이를 체크 후 배열길이만큼 반복
    // (2) 빈 배열로 설정한 cookies 에 값을 넣음.(let cookies = [];)
    ...
    */
    ...
    for (let i = 0; i < cookie_image_files.length; i++) {
        slider_panel.innerHTML +=  `<div class="swiper-slide">
                            <div id="slider_text${i}" class="slider_text">
                                <span style="color:#fff; font-weight:bold; font-size:2em;">${cookies[i].name}</span><br><br>
                                ${cookies[i].content}<br><br>
                                <b>￦ ${cookies[i].price}</b>
                            </div>	                                    
